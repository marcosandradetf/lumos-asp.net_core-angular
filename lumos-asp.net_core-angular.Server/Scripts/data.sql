...
--IF NOT EXISTS(SELECT 1 FROM TB_GROUPS)
...
--IF NOT EXISTS(SELECT 1 FROM TB_TYPES)
...
--GO
---- script company
--IF NOT EXISTS(SELECT 1 FROM TB_COMPANIES)
--insert into tb_companies (company_name)
--values('SCL Solutions');
--GO
---- script deposit
--IF NOT EXISTS(SELECT 1 FROM tb_deposits)
--insert into tb_deposits (deposit_name)
--values('Galpão BH');
--GO
-- script roles
IF NOT EXISTS(SELECT 1 FROM tb_roles)
INSERT INTO tb_roles (RoleEnum)
VALUES
    (1),
    (2),
    (3);
--GO
-- SCRIPT MATERIAIS
--IF NOT EXISTS(SELECT 1 FROM tb_materials)
